...
cls = SVC(C=0.9, cache_size=1500, class_weight=None, coef0=0.0,
...
while(True):
    ret, frame = cap.read()
    frame = cv2.resize(frame, (250, 150), interpolation = cv2.INTER_CUBIC)
    ...
    for i in range(height):
        for j in range(width):
            if(result[(i * width)+j]==1):
                img[i,j] = (0,0,255)
    img = cv2.resize(img, (260,150), interpolation = cv2.INTER_CUBIC)
